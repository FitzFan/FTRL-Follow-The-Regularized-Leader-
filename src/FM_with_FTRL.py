...
class FM_FTRL_machine(object):   
    def __init__(self, fm_dim, fm_initDev, L1, L2, L1_fm, L2_fm, D, alpha, beta, alpha_fm = .1, beta_fm = 1.0, dropoutRate = 1.0):
        ...
        self.dropoutRate = dropoutRate  # dropout rate (which is actually the inclusion rate), i.e. dropoutRate = .8 indicates a probability of .2 of dropping out a feature. dropout feature with probability of 1 - dropoutRate.
    ...
    def update(self, x, p, y):
        ...
        # 这里求解FM的交互项的方式比较笨拙，可以参考TensorFlow基于平方和公式化简后的求解，化简后的求解速度更快。
        for i in x:
            for k in range(self.fm_dim):
                g_fm = g * fm_sum[i][k]
