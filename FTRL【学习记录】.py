...
- 除了使用L1正则项，和RDA一样会使用L2范数作为额外项，来实现一些目标；
...
-- 其中，保留了RDA-L1中关于累积梯度的项，这项可以看作是当前特征对损失函数的贡献的一个估计【累积梯度越大，贡献越大。】
-- 由于使用了累积梯度，即使某一次迭代使某个重要特征约束为0，但如果后面这个特征变得稠密，它的参数又会变为非0；
-- **FTRL的巧妙之处在于**：在MSE的前面乘以了一个和learning_rate有着神奇关系的参数σ_s。因为这个参数，保证了FTRL在不使用L1时和SGD保持了一致性。
